...
// Dns H
var acrDnsZoneName = 'privatelink.${deployment().location}.cs.aks.containerservice.azure.us'
var aksDnsZoneName = 'privatelink.${deployment().location}.cx.aks.containerservice.azure.us'
...
module keyvault 'modules/keyvault/kv.bicep' = [for keyvault in keyvaults:{
...
module kvsecrets 'modules/keyvault/kvsecret.bicep' = [for secret in secrets: {
  scope: resourceGroup(rg.name)
  name: secret.name
  params: {
    kvName: secret.keyvault
...
module jumpbox 'modules/VM/virtualMachine.bicep' = [for vm in vms: {
  scope: resourceGroup(rg.name)
  name: vm.name
  params: {
    vmsettings: vm
    subnetId: subnetVM.id
    adminPassword: rKeyVault.getSecret(vm.secretName)
  }
  dependsOn: [
    keyvault
  ]
